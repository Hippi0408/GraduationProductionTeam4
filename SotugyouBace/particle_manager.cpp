...
void CParticleManager::LoadText(std::string path)
{
	...
	{	// 一行ずつ読み込む
		...
		ReadVector2(ss, "Scale", bundleData[tag[dataIndex]][index].info.scale);									// 大きさ
		...
		ReadVector2(ss, "Scaling", bundleData[tag[dataIndex]][index].info.scalingValue);						// 拡縮量
...
//＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝
// テキストファイルのベクター2Dの読み込み
//＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝
void CParticleManager::ReadVector2(std::stringstream& ss, std::string name, D3DXVECTOR2& vec)
{
