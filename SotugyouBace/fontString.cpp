...
const float CFontString::SCALE_SPEED = 0.4f;	// 拡大速度
const float CFontString::SCALE_SIZE = 5.0f;		// 拡大サイズ
const float CFontString::ANCHOR_DAMPING = 0.1f;	// アンカーの減衰速度
const float CFontString::MIMI_BLINK = 0.5f;		// 点滅の最大縮小サイズ
...
void CFontString::SizeScale(void)
{
	...
	{
		// 文字を拡大させる
		m_fScale += SCALE_SPEED;

		// 文字が一定の値まで拡大した場合
		if (m_fScale > SCALE_SIZE)
		{
...
void CFontString::SetFont()
{
	...
	{
		m_apFont[nCnt]->SetPos(m_pos + D3DXVECTOR3((m_size.x + m_size.x * 0.5f) * nCnt, 0.0f, 0.0f));
...
// 文字列の設定処理
//=====================================
void CFontString::SetString(const std::string letter, const PRIORITY priority)
{
	// string型の文字数を取得
	...
	for (int nLatter = 0; nLatter < nTex; nLatter += nStrSize)
	{
		...
		else
		{
			...
			else
			{
				...
				m_apFont[nCounter]->SetPos(m_pos + D3DXVECTOR3(fStrLenPos, 0.0f, 0.0f));
			}
...
// 生成処理
//=====================================
CFontString* CFontString::Create(const D3DXVECTOR3 pos, const D3DXVECTOR2 size, const std::string letter, const PRIORITY priority)
{
